...
modal_app = modal.App(
    "code_interpreter"
)  # creates container to store all info for code interpreter
...
test_cases = [
    ...
    {"input": [[-5, -1, -10, -3]], "expected_output": -3},  # Negative numbers
    {
        "input": [[3, 3, 5, 5, 7, 7]],
        "expected_output": 5,
    },  # Duplicates with a valid second largest
]
...
def extract_function(user_code: str):
    try:
        ...
        for node in ast.walk(
            parsed_code
        ):  # checks each node in the tree which is each line of code in user_code
            if isinstance(
    ...
    try:
        ...
        for test_case in test_cases:
            try:
                ...
                ):  # redirecting the output of the code to a buffer
                    result = user_func(
                        *test_case["input"]
                ...
                logging.info(
                    f"Test case passed: {result == test_case['expected_output']}"
                )  # logs if the test case passed or failed
            except Exception as e:
                results.append(
                    {
                        "input": test_case["input"],
